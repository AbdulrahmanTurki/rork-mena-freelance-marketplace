...
-- IMPORTANT: Only creates wallet when status changes from non-approved to approved
CREATE OR REPLACE FUNCTION create_seller_wallet()
RETURNS TRIGGER AS $$
BEGIN
  -- Only create wallet when status changes TO 'approved' (not when rejecting or creating)
  ...
  -- If status changes to 'rejected', ensure user_type is 'buyer'
...
-- Trigger for seller wallet creation and user_type management
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '====================================================';
  RAISE NOTICE '✓ COMPLETE UNIFIED SCHEMA SETUP';
  RAISE NOTICE '====================================================';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '✓ Tables: % / %', table_count, array_length(expected_tables, 1);
  ...
  RAISE NOTICE '  1. Run seed-categories.sql (if needed)';
  RAISE NOTICE '  2. Create admin user (see 04-create-admin.sql)';
  RAISE NOTICE '====================================================';
  RAISE NOTICE '';
END $$;
