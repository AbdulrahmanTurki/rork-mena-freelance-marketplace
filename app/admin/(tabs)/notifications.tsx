...
import { Bell, AlertCircle, Users, Shield, CheckCircle, X } from 'lucide-react-native';

interface Notification {
  id: string;
  type: 'verification' | 'dispute' | 'user' | 'system';
  title: string;
  message: string;
  isRead: boolean;
  createdAt: string;
...
export default function AdminNotificationsScreen() {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      id: '1',
      type: 'verification',
      title: isRTL ? 'طلب توثيق جديد' : 'New Verification Request',
      message: isRTL ? 'طلب توثيق بائع جديد في انتظار المراجعة' : 'New seller verification request pending review',
      isRead: false,
    ...
    },
    {
      id: '2',
      type: 'dispute',
      title: isRTL ? 'نزاع جديد' : 'New Dispute',
      message: isRTL ? 'نزاع جديد في الطلب #12345 يحتاج إلى إجراء' : 'New dispute in order #12345 requires action',
      isRead: false,
    ...
    },
    {
      id: '3',
      type: 'user',
      title: isRTL ? 'مستخدم جديد' : 'New User',
      message: isRTL ? '5 مستخدمين جدد انضموا اليوم' : '5 new users joined today',
      isRead: true,
  ...
  const getNotificationIcon = (type: Notification['type']) => {
    switch (type) {
      case 'verification':
        return <Shield size={20} color={theme.primary} />;
      case 'dispute':
        return <AlertCircle size={20} color={theme.primary} />;
      case 'user':
        return <Users size={20} color={theme.primary} />;
      default:
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { backgroundColor: theme.headerBackground, paddingTop: insets.top + 16 }]}>
