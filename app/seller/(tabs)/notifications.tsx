...
import { Bell, Package, ShoppingBag, MessageCircle, CheckCircle, X, DollarSign } from 'lucide-react-native';

interface Notification {
  id: string;
  type: 'order' | 'message' | 'gig' | 'payout' | 'system';
  title: string;
  message: string;
  isRead: boolean;
  createdAt: string;
...
export default function SellerNotificationsScreen() {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      ...
      message: isRTL ? 'لديك طلب جديد بقيمة 500 ريال' : 'You have a new order worth SAR 500',
      isRead: false,
    ...
    },
    {
      id: '2',
      type: 'payout',
      title: isRTL ? 'سحب ناجح' : 'Payout Successful',
      message: isRTL ? 'تم تحويل 2,500 ريال إلى حسابك البنكي' : 'SAR 2,500 has been transferred to your bank account',
      isRead: false,
    ...
    },
    {
      ...
      message: isRTL ? 'رسالة جديدة من العميل في الطلب #12345' : 'New message from client in order #12345',
      isRead: true,
  ...
  const getNotificationIcon = (type: Notification['type']) => {
    switch (type) {
      ...
      case 'payout':
        return <DollarSign size={20} color={theme.primary} />;
      default:
